...
class BaseBackbone(nn.Module):
    ...
    def update_memory(self, z_clone, x, x_clone, templates_feat_len, search_feat_len, templates_num, prev_decision,
        ...
        for i, blk in enumerate(self.blocks):
            ...
            if i == 3 and topk_mem_index is not None:
                topk_mem = []
                for j in range(B):
    ...
    def forward_features(self, z, x, template_mask, search_feat_len, templates_feat_len_org, threshold, tgt_type, train_num, template_show, update=False, track_query=None, token_type="add"):
        ...
        for i, blk in enumerate(self.blocks):
            x, prev_decision, policy, topk_mem_index, mask_show = blk(x=x, templates_feat_len=new_templates_feat_len, search_feat_len=search_feat_len, templates_num=T,
                               prev_decision=prev_decision, policy=policy, templates_feat_len_org=templates_feat_len_org)
