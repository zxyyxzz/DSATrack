...
from lib.models.dsatrack.base_backbone_stu import BaseBackbone
from lib.utils.blocks import Encoder4Dgnn
keep_index = None

class Attention(nn.Module):
    def __init__(self, dim, num_heads, qkv_bias, attn_drop=0., proj_drop=0., residual=False, feature_size_z=64, feature_size_x=256, relevance_attn=False, layer=None, using_att_weights=False, keep_num=None, early=True, norm=False,training=True, first_dm_layer=3):
        super().__init__()
        self.dim = dim
        ...
        self.layer = layer
        ksize_4D = 3
        self.first_dm_layer = first_dm_layer
    ...
    def forward(self, x, z_size, x_size, t, prev_decision=None, policy=None, noNorm_x=None, dm=False, train_num=1, templates_feat_len_org=64):
        ...
        if self.relevance_attn:
            ...
            if self.is_training:
                ...
                if self.layer == self.first_dm_layer and dm:
...
class Block(nn.Module):
    def __init__(self, dim, num_heads, mlp_ratio=4., qkv_bias=False, drop=0., attn_drop=0., drop_path=0.,
                 ...
                 relevance_attn=False, using_att_weights=False, keep_num=None, layer=None, training=True, first_dm_layer=3):
        super().__init__()
        self.norm1 = norm_layer(dim)
        self.attn = Attention(dim, num_heads=num_heads, qkv_bias=qkv_bias, attn_drop=attn_drop, proj_drop=drop,
                              residual=residual, feature_size_z=feature_size_z, feature_size_x=feature_size_x,
                              relevance_attn=relevance_attn, layer=layer, using_att_weights=using_att_weights, keep_num=keep_num, training=training, first_dm_layer=first_dm_layer)
    ...
    def forward(self, x, templates_feat_len, search_feat_len, templates_num, prev_decision, policy, dm=False, train_num=1, templates_feat_len_org=64, remove_rate=1.0):
        ...
        if filter_x is not None:
           x = filter_x
        x = x + remove_rate * self.drop_path(feat)
        x = x + remove_rate * self.drop_path(self.mlp(self.norm2(x)))
...
class VisionTransformer(BaseBackbone):
    ...
    def __init__(self, img_size=224, patch_size=16, in_chans=3, num_classes=1000, embed_dim=768, depth=12,
                 ...
                 relevance_attn=None, using_att_weights=None, training=True, add_cls_token=False, remove_layers=[]):
        """
        Args:
        ...
        self.first_dm_layer = relevance_attn[0][0]
        ...
        self.blocks = nn.Sequential(*[
            Block(
                ...
                residual=residual, feature_size_z=feature_size_z, feature_size_x=feature_size_x,
                relevance_attn=relevance_attn_layer[i], using_att_weights=using_att_weights[i], keep_num=keep_num[i], layer=i, training=self.is_training, first_dm_layer=self.first_dm_layer)
...
def vit_base_depth12(pretrained=False, **kwargs):
    """
...
def vit_base_depth8(pretrained=False, **kwargs):
    """
    ViT-Base model (ViT-B/16) from original paper (https://arxiv.org/abs/2010.11929).
    """

    model_kwargs = dict(
        patch_size=16, embed_dim=768, depth=8, num_heads=12, **kwargs)
    model = _create_vision_transformer('vit_base_patch16_224_in21k', pretrained=pretrained, **model_kwargs)
    return model


def vit_base_depth7(pretrained=False, **kwargs):
    """
    ViT-Base model (ViT-B/16) from original paper (https://arxiv.org/abs/2010.11929).
    """

    model_kwargs = dict(
        patch_size=16, embed_dim=768, depth=7, num_heads=12, **kwargs)
    model = _create_vision_transformer('vit_base_patch16_224_in21k', pretrained=pretrained, **model_kwargs)
    return model


def vit_base_depth6(pretrained=False, **kwargs):
    """
    ViT-Base model (ViT-B/16) from original paper (https://arxiv.org/abs/2010.11929).
    """
...
def vit_base_depth4(pretrained=False, **kwargs):
    """
    ViT-Base model (ViT-B/16) from original paper (https://arxiv.org/abs/2010.11929).
    """

    model_kwargs = dict(
        patch_size=16, embed_dim=768, depth=4, num_heads=12, **kwargs)
    model = _create_vision_transformer('vit_base_patch16_224_in21k', pretrained=pretrained, **model_kwargs)
    return model    


def vit_tiny_depth12(pretrained=False, **kwargs):
    """
     ViT-Tiny (Vit-Ti/16)
    """
    model_kwargs = dict(
...
def vit_tiny_depth7(pretrained=False, **kwargs):
    """
     ViT-Tiny (Vit-Ti/16)
    """
    model_kwargs = dict(
        patch_size=16, embed_dim=192, depth=7, num_heads=3, **kwargs)
    model = _create_vision_transformer('vit_tiny_patch16_224_in21k', pretrained=pretrained, **model_kwargs)
    return model


def deit_tiny_stu(pretrained=False, **kwargs):
