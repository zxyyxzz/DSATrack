...
class FocalLoss(nn.Module, ABC):
    ...
    def forward(self, prediction, target):
        ...
        if prediction.shape[1] == 2:
            ...
            prediction_negative = prediction[:, 1:]
        else:
            prediction_positive = prediction
            prediction_negative = 1-prediction
        positive_loss = torch.log(prediction_positive) * torch.pow(1 - prediction_positive, self.alpha) * positive_index
        negative_loss = torch.log(prediction_negative) * torch.pow(1 - prediction_negative,
