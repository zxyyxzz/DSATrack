...
class LTRTrainer(BaseTrainer):
    def __init__(self, actor, loaders, optimizer, settings, lr_scheduler=None, remove_mode=False):
        """
        Args:
    ...
    def _print_stats(self, i, loader, batch_size):
        ...
        fps = batch_size / (current_time - prev_frame_time_backup)
        data_time = self.data_read_done_time - prev_frame_time_backup
        gpu_trans_time = (self.data_to_gpu_time - self.data_read_done_time)
        forward_time = current_time - self.data_to_gpu_time
        print("step={}, batch_size={}, data_time={:.2f}, gpu_trans_time={:.2f}, forward_time={:.2f}, fps={:.2f}".format(i, batch_size, data_time, gpu_trans_time, forward_time, fps))

        if i == loader.__len__() or i % self.settings.print_interval == 0:
            print_str = '[%d: %d/%d]' % (self.epoch, i, loader.__len__())
