...
def sample_target(im, target_bb, search_area_factor, output_sz=None, mask=None):
    ...
    if output_sz is not None:
        ...
        if mask is None:
            return im_crop_padded, resize_factor, None
        # mask_crop_padded = \
        # F.interpolate(mask_crop_padded[None, None], (output_sz, output_sz), mode='bilinear', align_corners=False)[0, 0]
        return im_crop_padded, resize_factor, None, None

    else:
        if mask is None:
            return im_crop_padded, None, 1.0
        return im_crop_padded, 1.0, None, None
